...
def train_one_epoch(args: Namespace, model: torch.nn.Module, criterion: torch.nn.Module,
  ...
  for data_iter_step, batch in tqdm(
    ...
    samples_t = samples_t.reshape(
        batch_size, in_channels, num_temporal_frame, int(
            block_width ** 2), frame_size, frame_size)
    samples_t = samples_t.reshape(
        batch_size,
        in_channels,
        num_temporal_frame,
    ...
    samples_t = samples_t.reshape(
        batch_size * in_channels * num_temporal_frame,
        block_width * frame_size,
        block_width * frame_size)
    samples_t = resize(samples_t).reshape(
        batch_size,
        in_channels,
        num_temporal_frame,
        frame_size,
    ...
    if samples.shape[2] != 1:
      ...
      samples = samples.reshape(
          batch_size * (8 + num_temporal_frame), in_channels, frame_size, frame_size)
    ...
    with torch.cuda.amp.autocast():
      ...
      targets = nn.functional.one_hot(
          targets, num_classes=output.shape[1]).to(
          torch.float32)
  ...
  for batch in metric_logger.log_every(data_loader, 100, header):
    ...
    with torch.cuda.amp.autocast():
      ...
      target = nn.functional.one_hot(
          target, num_classes=output.shape[1]).to(
          torch.float32)
...
def final_test(data_loader, model, device, file, args):
  ...
  for batch in metric_logger.log_every(data_loader, 100, header):
    ...
    samples_t = samples_t.reshape(
        batch_size, image_channel, num_temporal_frame, int(
            block_width ** 2), image_size, image_size)
    samples_t = samples_t.reshape(
        batch_size,
        image_channel,
        num_temporal_frame,
    ...
    if num_frames != 1:
      images = images.permute(0, 2, 1, 3, 4)
      images = images.reshape(
          batch_size * (8 + num_temporal_frame), image_channel, image_size, image_size)
    ...
    for i in range(output.size(0)):
      string = "{} {} {} {} {}\n".format(
          ids[i], str(
              output.data[i].cpu().numpy().tolist()), str(
              int(
                  target[i].cpu().numpy())),
          str(int(chunk_nb[i].cpu().numpy())), str(
