...
def _download(url: str, root: str):
  ...
  if os.path.isfile(download_target):
    if hashlib.sha256(open(download_target, "rb").read()
...
def load(name: str, device: Union[str, torch.device] = "cuda" if torch.cuda.is_available() else "cpu",
         jit: bool = False, download_root: str = None, num_classes: int = 400, num_frame: int = 9):
  """Load a CLIP model
  ...
  if not jit:
    ...
    model = build_model(
        ...
        num_class,
        num_frame=num_frames).to(device)
    if str(device) == "cpu":
      model.float()
    ...
    def patch_float(module):
      ...
      for graph in graphs:
        for node in graph.findAllNodes("aten::to"):
          ...
          for i in [
                  1, 2]:  # dtype can be the second or third argument to aten::to()
