...
class TemporalPositionalEmbedding(nn.Module):
  ...
  def forward(self, tensor):
    if len(tensor.shape) == 3:
      ...
      emb = torch.cat(
          [torch.tensor([[0.0]] * Tt).to(tensor.device), emb], dim=1)  # [Tt, N]
      emb = emb.view(-1)  # [TtxN]
      emb = torch.einsum(
          "i,j->ij",
          emb,
...
class ModifiedResNet(nn.Module):
  ...
  def __init__(self, layers, output_dim, heads,
...
class ResidualAttentionBlock(nn.Module):
  def __init__(self, d_model: int, n_head: int, num_frames: int,
               attn_mask: torch.Tensor = None, adapter: str = None):
    super().__init__()
    ...
    if self.adapter == 'w-adapter':
      ...
      self.s_adapter_attn = Adapter(d_model=d_model, bottleneck=int(
          0.25 * d_model), dropout=0.1, adapter_layernorm_option=None)
      self.t_adapter_attn = Adapter(d_model=d_model, bottleneck=int(
          0.25 * d_model), dropout=0.1, adapter_layernorm_option=None)
  ...
  def forward(self, x: torch.Tensor):
    if self.adapter is None:
      ...
      x = torch.cat([x_t.reshape(N, B, -1, D),
                    x_s.reshape(N, B, -1, D)], dim=2).reshape(N, -1, D)
      return x
...
def build_model(state_dict: dict, num_class: int, num_frame: int):
  ...
  if vit:
    ...
    counts: list = [
        len(
            set(
                k.split(".")[2] for k in state_dict if k.startswith(f"visual.layer{b}"))) for b in [
            1,
            2,
            3,
            4]]
  ...
  model = CLIP(
      ...
      num_class=num_class, num_frame=num_frame
  )
  ...
  msg = load_checkpoint(model, state_dict)
...
def load_checkpoint(model, state_dict, strict=False):
