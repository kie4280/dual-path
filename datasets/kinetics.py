...
class VideoClsDataset(Dataset):
    ...
    if self.mode == 'train':
      ...
      if args.num_sample > 1:
        ...
        if self.data_transform is None:
          ...
          sampled_list = [Image.fromarray(buffer[vid, :, :, :]).convert(
              'RGB') for vid in range(buffer.shape[0])]
      ...
      buffer = self.loadvideo_decord(sample)
      # print(buffer.shape)
      # self.clip_len = self.args.num_sample

      while len(buffer) == 0:
        _logger.warn("video {}, temporal {}, spatial {} not found during testing".format(
  ...
  def loadvideo_decord(self, sample, sample_rate_scale=1):
    ...
    if os.path.getsize(fname) < 1 * 1024:
      _logger.info('SKIP: ', fname, " - ", os.path.getsize(fname))
      return []
    try:
      ...
      _logger.warning("video cannot be loaded by decord. Reason:", str(e))
      return []
    ...
    if self.mode == 'train':
      sample_rate_candidate = [1, 2, 4, 8]
      self.frame_sample_rate = random.choice(sample_rate_candidate)
...
def spatial_sampling(
  ...
  if spatial_idx == -1:
    ...
    frames = video_transforms.grayscale(0.2, frames)
    frames = video_transforms.color_jitter(frames)
  else:
...
class VideoMAE(torch.utils.data.Dataset):
  ...
  def __getitem__(self, index):
    ...
    if self.video_loader:
      if '.' in directory.split('/')[-1]:
        ...
        # So we need to provide extension (i.e., .mp4) to complete the file
        # name.
        video_name = '{}.{}'.format(directory, self.video_ext)
      # video_name = os.path.join(self.setting, video_name)
    ...
    process_data = process_data.view((self.new_length,
                                      3) + process_data.size()[-2:]).transpose(0,
  ...
  def _video_TSN_decord_batch_loader(
    ...
    try:
      ...
      sampled_list = [Image.fromarray(video_data[vid, :, :, :]).convert(
          'RGB') for vid, _ in enumerate(frame_id_list)]
...
def build_training_dataset(args):
  ...
  dataset = VideoMAE(
      ...
      use_decord=True,
      lazy_init=False)
  _logger.info("Data Aug = %s" % str(transform))
  return dataset
